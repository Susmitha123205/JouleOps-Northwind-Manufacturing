...
import os 
fake = Faker() 
PLANTS = ['PLT-PUN', 'PLT-CHE', 'PLT-HYD', 'PLT-COI']
...
def generate_sales_orders(materials, customers):
    ...
    for i in range(1, 801):
        ...
        sales_orders.append({
            ...
            'region': random.choice(REGIONS),
            'created_on': fake.date_time_between(start_date='-3m', end_date='now'),
        })
...
def generate_invoices(customers):
    ...
    for i in range(1, 401):
        ...
        customer = random.choice(customers['customer_id'].tolist())
        due_date = fake.date_between(start_date='-90d', end_date='+30d')
        days_overdue = 0
...
def generate_tickets():
    return pd.DataFrame(columns=[
        'ticket_id', 'equipment_id', 'material_id', 'priority',
        'assigned_team', 'description', 'status', 'created_on', 'created_by_role'
    ])
def generate_audit_log():
    return pd.DataFrame(columns=[
        'log_id', 'ts', 'user_role', 'tool_name', 'params_masked', 'outcome'
    ])
...
def main():
    ...
    sales_orders_df = generate_sales_orders(materials_df, customers_df)
    invoices_df = generate_invoices(customers_df)
    tickets_df = generate_tickets()
    audit_log_df = generate_audit_log()

    materials_df.to_csv('data/data_materials.csv', index=False)
    customers_df.to_csv('data/data_customers.csv', index=False)
    sales_orders_df.to_csv('data/data_sales_orders.csv', index=False)
    invoices_df.to_csv('data/data_invoices.csv', index=False)
    tickets_df.to_csv('data/data_tickets.csv', index=False)
    audit_log_df.to_csv('data/data_audit_log.csv', index=False)
